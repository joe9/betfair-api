...
import Protolude hiding (bracket, error, filter, finally)
...
getMarketCatalogue :: AppKey -> UserName -> Password -> IO ()
getMarketCatalogue appKey username password = do
  manager <- newManager (managerSetProxy noProxy tlsManagerSettings)
  let context = initializeContext manager appKey (Just putStr)
  bracket
    (fmap
       (\token -> context {cToken = token})
       (sessionToken context username password))
    (\updatedContext -> putText "Logging out" >> logout updatedContext)
    (\updatedContext -> do
       print (cToken updatedContext)
       _ <- (tennisAndCricketMarketCatalogues updatedContext >>= print)
       --                    (_,_) <-
       --                      concurrently
...
marketCatalogueParams =
  ...
  { filter =
      defaultMarketFilter
      { eventTypeIds = (Just . fmap show) ([2 :: Integer, 4])
...
tennisAndCricketMarketCatalogues :: Context -> IO [MarketCatalogue]
